...
{
public:
    AsyncLogger() : m_active(false), m_buffer_accumulating(m_capacity),  m_buffer_flushing(m_capacity) {}
    ~AsyncLogger() {
        Stop();
    }
    ...
    void Thread(void)
    {
        ...
        while (true) {
...

            {
                ...
                if (size > m_capacity) {
                    LogInstance().LogPrintStr(strprintf(
                ...
                for (; n_to_flush > 0; --n_to_flush) {
                    LogInstance().LogPrintStr(std::move(m_buffer_flushing[flush_idx]));
                    flush_idx = (flush_idx + 1) % m_capacity;
                }
                LogInstance().FlushFile();
            }
        }
    }
